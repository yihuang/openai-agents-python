    ...
    async def execute_tools_and_side_effects(
        ...
        *,
        ...
        if check_tool_use.is_final_output:
            # Log a warning if the output type is not the same as the agent's output type
            if (
                agent.output_type
                and check_tool_use.final_output is not None
                and not isinstance(check_tool_use.final_output, agent.output_type)
            ):
                logger.warning(
                    f"The output type of the agent is {agent.output_type}, but tool use behavior"
                    f"returned a final output of type {type(check_tool_use.final_output)}"
                )
